...
class Zombie:
    ...
    def wander(self):
        ...
        if self.timer <= 0:
            self.timer = 10.0
            self.dir = random.random() * 2 * math.pi # 방향을 라디안으로 설정
    ...
    def find_player(self):
        # fill here
        distances2 = (server.boy.x - self.x)**2 + (server.boy.y - self.y)**2
        if distances2 <= (PIXEL_PER_METER*10)**2:
            print('Find Plater Success')
            return BehaviorTree.SUCCESS
        else:
            self.speed = 0
    ...
    def move_to_player(self):
        ...
        return BehaviorTree.SUCCESS # 일단, 소년쪽으로 움직이기만 해도 성공으로 한다.
        pass
    ...
    def get_next_position(self):
        ...
        print('Next Position Fount Success')
        return BehaviorTree.SUCCESS
        pass
    ...
    def build_behavior_tree(self):
        ...
        move_to_player_node = LeafNode('Move to Plater', self.move_to_player)
